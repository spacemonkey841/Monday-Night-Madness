...
class PlayState extends MusicBeatState
{
		...
		if (SONG.stage == null)
		{
			switch (storyWeek)
			{
				...
				case 5:
					...
					{
						stageCheck = 'bakeryEvil';
					}
					else if (songLowercase == 'eggnog')
					{
						stageCheck = 'bakeryWorry';
					}
					else
					{
						stageCheck = 'bakery';
				...
				case 'romp':
					{
						...
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('romp/Room'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);

						var doors:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('romp/Doors'));
						doors.antialiasing = true;
						doors.scrollFactor.set(0.9, 0.9);
						doors.active = false;
						add(doors);

						var vent:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('romp/Vent'));
						vent.antialiasing = true;
						vent.scrollFactor.set(0.9, 0.9);
						vent.active = false;
						add(vent);

						var pipes:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('romp/Pipes'));
				...
				case 'rompEvil':
					{
						...
						var hole:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('romp/Mustached Star Hole'));
						hole.antialiasing = true;
						hole.scrollFactor.set(0.9, 0.9);
						hole.active = false;
						add(hole);

						var rubble:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('romp/Wall Rubble'));
						rubble.antialiasing = true;
						rubble.scrollFactor.set(0.9, 0.9);
						rubble.active = false;
						add(rubble);
					}
				case 'railroad':
					{
						curStage = 'railroad';
							...
							var bg:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('railroad/Wall'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);

						var pipes:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('railroad/Pipes'));
						pipes.antialiasing = true;
						pipes.scrollFactor.set(0.9, 0.9);
						pipes.active = false;
						add(pipes);

						var posters:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('railroad/Posters'));
						posters.antialiasing = true;
						posters.scrollFactor.set(0.9, 0.9);
						posters.active = false;
						add(posters);

						var tunnelBack:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('railroad/Tunnel'));
						tunnelBack.antialiasing = true;
						tunnelBack.scrollFactor.set(0.9, 0.9);
						tunnelBack.active = false;
						add(tunnelBack);

						var tracks:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('railroad/Train Tracks'));
						tracks.antialiasing = true;
						tracks.scrollFactor.set(0.9, 0.9);
						tracks.active = false;
						add(tracks);

						phillyTrain = new FlxSprite(6000, 280).loadGraphic(Paths.image('railroad/Train'));
						...
						var tunnelFront:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('railroad/Tunnel Front'));
						tunnelFront.antialiasing = true;
						tunnelFront.scrollFactor.set(0.9, 0.9);
						tunnelFront.active = false;
						add(tunnelFront);

						var ground:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('railroad/Ground'));
				...
				case 'train':
					{
						...
						var bg:FlxSprite = new FlxSprite(-200, -50).loadGraphic(Paths.image('train/sky'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);

						var ground:FlxSprite = new FlxSprite(-120, -50);
						ground.frames = Paths.getSparrowAtlas('train/ground');
						ground.animation.addByPrefix('drive', "Ground Moving", 24);
						ground.animation.play('drive');
						ground.scrollFactor.set(0.4, 0.4);
						add(ground);

						var train:FlxSprite = new FlxSprite(-120, -50);
						train.frames = Paths.getSparrowAtlas('train/train');
						train.animation.addByPrefix('drive', "Train", 24);
						train.animation.play('drive');
						add(train);

						var agents:FlxSprite = new FlxSprite(-120, -50);
						agents.frames = Paths.getSparrowAtlas('train/agentsBop');
						agents.animation.addByPrefix('bop', "Agents Bop", 24);
						agents.animation.play('bop');
						if (FlxG.save.data.distractions)
						{
							add(agents);							
						}
				...
				case 'island':
					{
						...
						var bg:FlxSprite = new FlxSprite(-600, -800).loadGraphic(Paths.image('island/Sky'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);

						var island:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('island/islandBase'));
						island.antialiasing = true;
						island.scrollFactor.set(0.9, 0.9);
						island.active = false;
						add(island);

						var platform:FlxSprite = new FlxSprite(-600, -600).loadGraphic(Paths.image('island/platform'));
						platform.antialiasing = true;
						platform.scrollFactor.set(0.9, 0.9);
						platform.active = false;
						add(platform);

						crowdBoppers = new FlxSprite(-600, -600);
						crowdBoppers.frames = Paths.getSparrowAtlas('island/bgCrowd');
						crowdBoppers.animation.addByPrefix('bop', "bgCrowdBop", 24, false);
						crowdBoppers.antialiasing = true;
						if (FlxG.save.data.distractions)
						{
							add(crowdBoppers);
						...
						var controll:FlxSprite = new FlxSprite(-600, -560).loadGraphic(Paths.image('island/platformControll'));
						controll.antialiasing = true;
						controll.scrollFactor.set(0.9, 0.9);
				...
				case 'island2':
					{
						...
						var controll:FlxSprite = new FlxSprite(-600, -600).loadGraphic(Paths.image('island/platformControll'));
						controll.antialiasing = true;
						controll.scrollFactor.set(0.9, 0.9);
						controll.active = false;
						add(controll);
				...
				case 'islandEvil':
					{
						...
						var hole:FlxSprite = new FlxSprite(-600, -600).loadGraphic(Paths.image('island/trickyHole'));
						hole.antialiasing = true;
						hole.scrollFactor.set(0.9, 0.9);
						hole.active = false;
						add(hole);
						...
						crowdBoppers = new FlxSprite(-600, -600);
						crowdBoppers.frames = Paths.getSparrowAtlas('island/skeletonCrowd');
						crowdBoppers.animation.addByPrefix('bop', "skeletonCrowdBop", 24, false);
						crowdBoppers.antialiasing = true;
						if (FlxG.save.data.distractions)
						{
							add(crowdBoppers);
				...
				default:
					{
						defaultCamZoom = 0.9;
						curStage = 'tree';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('tree/Sky'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
	
						var ground:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('tree/Grass'));
						ground.antialiasing = true;
						ground.scrollFactor.set(0.9, 0.9);
						ground.active = false;
						add(ground);

						var tree:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('tree/Tree'));
						tree.antialiasing = true;
						tree.scrollFactor.set(0.9, 0.9);
						tree.active = false;
						add(tree);

						var cannon:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('tree/Cannon'));
						cannon.antialiasing = true;
						cannon.scrollFactor.set(0.9, 0.9);
						cannon.active = false;
						add(cannon);
					}
			}
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		var red:FlxSprite = new FlxSprite(-100, -100).makeGraphic(FlxG.width * 2, FlxG.height * 2, 0xFFbb0000);
		red.scrollFactor.set();

		var senpaiEvil:FlxSprite = new FlxSprite();
		senpaiEvil.frames = Paths.getSparrowAtlas('island/Auditor_Dies_Assets');
		senpaiEvil.animation.addByPrefix('idle', 'auditor dies', 24, false);
	...
	override function beatHit()
	{
		...
		if (curSong == 'Tutorial' && dad.curCharacter == 'grunt')
		{
		...
		switch (curStage)
		{
			...
			case 'limo':
				if (FlxG.save.data.distractions)
				{
					agents.animation.play('bop', true);
				}
